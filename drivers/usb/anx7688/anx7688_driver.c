...
unsigned char auto_update = 1; 
static unsigned short int OcmFwVersion = 0;
#endif
unsigned int ptn_c1 = C1C2_HIGH;  
...
static void anx7688_get_max_rdo()
{
	...
	pdo_data.pd_list[0][0] = RDO_Max_Voltage * 100;	
	pdo_data.pd_list[0][1] =
		((RDO_Max_Power * 500) / RDO_Max_Voltage) * 10;  
...
int anx7688_get_prop(u8 prop)
{
	...
	switch (prop) {
		...
		case ANX_PMODE: 
			return data->pmode;
		case ANX_PROLE_CHANGE: 
			return data->pr_change;
		case ANX_VCONN: 
			return data->vconn;
		case ANX_START_HOST_FLAG: 
		case ANX_EMARKER: 
		case ANX_NON_STANDARD: 
			return -EINVAL;
		case ANX_PD_CAP: 
