...
#define MAX_WRITE_CYCLE_TIME 5000 
...
void ReadPage(unsigned short int Address,unsigned char *pData)
{
    ...
    if(WriteBlockReg( 0xE0, 2, AddrBuf) < 0) { 
        pr_info("NAK!\n");
        return;
    }
    ...
    do {
        RegData=ReadReg( 0xE2 ); 
    } while(BITTESTFALSE(RegData, 3)) ; 
    if(ReadBlockReg( 0xD0, 16, DataBuf) < 0) { 
        pr_info("NAK!\n");
        return;
    }
...
void WritePage(unsigned short int Address,unsigned char *pData)
{
    ...
    if(WriteBlockReg(0xD0, 16, WriteDataBuf) < 0) { 
        pr_info("NAK!\n");
        return;
    }
    WriteReg(0xE2, 0x01); 
    ...
    if(ReadBlockReg( 0xD0, 16, ReadDataBuf) < 0) { 
        pr_info("NAK!\n");
        return;
    }
...
#define SEGDATA_ADDR (0x0890 + 0x10) 
...
unsigned short int burnhexauto(void)
{
    ...
    do {
        ...
        if(AutoUpdateRetry == 0)
            BurnHexWriteCycleTime = MAX_WRITE_CYCLE_TIME/2; 
#endif
#if USE_CRC_ERASE
        EraseSegCrc();
#endif
        ...
        BurnHexWriteCycleTime = MAX_WRITE_CYCLE_TIME; 
        ...
        if(GetLineData(LineBuf, &ByteCount, &Address, &RecordType, WriteDataBuf) == 0) {
            if(RecordType == 0) {
                if(Address== VERSION_ADDR ) {
                    ...
                    pData[1] = WriteDataBuf[VERSION_ADDR + 1 - Address] ; 
                    break;
                }
            } else if(RecordType == 1) {
