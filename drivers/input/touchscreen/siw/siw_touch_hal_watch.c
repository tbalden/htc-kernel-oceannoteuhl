...
struct ext_watch_query_font_position {	
	bool vertical_position_supported;
	bool horizontal_position_supported;
};

struct ext_watch_query_font_time {	
	bool h24_supported;
	bool ampm_supported;
};

__packed struct ext_watch_query_font_color {	
	u8 max_num;		
	bool lut_supported;
...
struct ext_watch_config_font_effect {
	...
	u32	zero_disp;				
	u32	clock_disp_type;		
	u32	midnight_hour_zero_en;	
	struct ext_watch_config_font_effect_blink blink;	
};
...
struct ext_watch_config_time_sync {	
	u32	len;
	u32	rtc_cwhour;		
	u32	rtc_cwmin;		
	u32	rtc_cwsec;		
...
__packed struct ext_watch_bits_ctrl {	
	u32	reserved0:1;
	u32	dispmode:1;	
	u32	reserved1:5;
	u32	alpha:9;
...
__packed struct ext_watch_bits_area { 	
	u32	watstart:12;
	u32	watend:12;
};

__packed struct ext_watch_bits_blink_area { 
	u32 bstartx:12;
	u32 bendx:12;
};

__packed struct ext_watch_bits_lut {	
	u8 b;
	u8 g;
	u8 r;
	u8 rssvd0;
...
__packed struct ext_watch_cfg_mode {	
	struct ext_watch_bits_ctrl watch_ctrl;				
	struct ext_watch_bits_area watch_area_x;			
	struct ext_watch_bits_area watch_area_y;			
	struct ext_watch_bits_blink_area blink_area;		
	struct ext_watch_bits_lut __lut[EXT_WATCH_LUT_NUM];	
	struct ext_watch_bits_lut *lut;
};

__packed struct ext_watch_cfg_time {	
	u32 disp_waton;						
	struct ext_watch_bits_time rtc_sct;	
	u32 rtc_sctcnt;						
	u32 rtc_mit;						
	u32 rtc_capture;					
	struct ext_watch_bits_time rtc_ctst;	
	u32 rtc_ecnt;						
};
...
__packed struct ext_watch_cfg_position {	
	u32 h10x_pos:9;
	u32 h1x_pos:9;
	u32 reserved0:14;
...
__packed struct ext_watch_cfg_status {	
	u32 step:3;
	u32 en:1;
	u32 en_24:1;
	u32 zero_en:1;
...
static int ext_watch_set_curr_time(struct device *dev)
{
	...
	u32 rtc_ctrl = 0;
	u32 rtc_sctcnt = 0;
	u32 rtc_count = EXT_WATCH_SET_RTC_COUNT;		 
	int ret = 0;
